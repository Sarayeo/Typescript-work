...
export class TaskManager {
    ...
    addTaskToCategory(task, categoryName) {
        const category = this.categories.find((c) => c.name === categoryName);
        if (!category) {
    ...
    search() {
        ...
        if (searchTerm) {
            searchedTasks = this.tasks.filter((task) => task.title.toLowerCase().includes(searchTerm) ||
    ...
    displaySearchedTasks(searchedTasks) {
        const tasksList = document.getElementById('task-list');
        if (tasksList) {
            tasksList.innerHTML = '';
